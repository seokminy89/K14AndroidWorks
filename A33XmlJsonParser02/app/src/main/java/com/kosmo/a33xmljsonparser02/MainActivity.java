...
public class MainActivity extends AppCompatActivity {
    ...
    //리소스에 저장된 Txt 파일을 IO스트림을 통해 연결한 후 내용을 읽어온다.
    ...
    private void getJsonParser()
    {
        ...
        try{
            ...
            for(int i =0 ; i < array.length() ; i++)
            {
                ...
                String printStr = String.format("이름:%s,나이:%s,취미:%s,아이디:%s,패스워드:%s",
            ...
            /*
            커스텀 어댑터 객체를 생성한다. 인자로 컨텍스트, 데이터를 저장한 컬렉션,
            커스텀 레이아웃 XML을 전달한다. 어댑터에서는 레이아웃을 전개하고, getView()에서
            리스트 컬렉션에 저장된 항목을 하나씩 반환하여 전체 리스트뷰를 구성하게 된다.
             */
            ...
            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    Toast.makeText(getApplicationContext(),"선택한배우:"+items.get(position).getName(),
