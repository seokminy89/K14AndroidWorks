...
public class MainActivity extends AppCompatActivity {
    ...
    private void getJsonData2(){
        ...
        try {
            ...
            String jsonPrint = String.format("top:%s,right:%s,bottom:%s,left:%s",top,right,bottom,left);
    ...
    형식
        next() : XML탐색을 위한 메서드로 XML요소를 식별하면 탐색을 중지하고
            식별된 요소를 반환한다.
    상수
        START_DOCUMENT : XML 문서의 시작으로 파싱의 시작을 알린다.
    ...
    private void getXmlData(){
        try {
            ...
            /*
            Arrays.asList()
                :인자로 주어진 배열을 List 컬렉션으로 변환해주는 메서드.
                단, 이렇게 변환된 컬렉션에는 원소를 새롭게 추가할수는 없다.
                하지만 컬렉션에서 제공하는 모든 메서드를 사용할 수 있는 장점이 있다.
             */
                ...
                switch (event){
                    ...
                    case XmlPullParser.TEXT:
                        if(currentTag!=null && tagList.contains(currentTag))
                        {
                            ...
                            if (currentTag.equals("number")){//번호
                                xNumber.add(value);
                            }
                            else if (currentTag.equals("actor")){//배우
                                xActor.add(value);
                            }
