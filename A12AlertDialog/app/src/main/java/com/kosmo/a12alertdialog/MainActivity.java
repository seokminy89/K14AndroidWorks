...
public class MainActivity extends AppCompatActivity {
    ...
    //체크박스, 라디오 대화상자에서 사용
    private String[] girlGroup = {"트와이스","AOA","모모랜드","블렉핑크"};
    //선택한 라디오 항목의 인덱스
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        progress.setIcon(android.R.drawable.ic_menu_day);//안드로이드 기본아이콘 사용
        progress.setTitle("KOSMO여러분들");
        progress.setMessage("열공하는중입니다. 조용히 합시다.");
        ...
        btnAlertProgress.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //버튼을 눌렀을때 대화상자가 열려있지 않을때만 띄워준다.
                if (!progress.isShowing()) {
                    progress.show();
                }
                //Handler객체를 통해 2초간 대기후 진행창 닫기
                //(이 예제에는 서버통신이 없기때문에 직접 닫아줘야함.)            }
